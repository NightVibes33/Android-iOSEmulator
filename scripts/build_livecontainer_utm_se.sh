...
echo "[5/10] Patching LiveContainer startup, APK flow and Xcode compatibility"
...
for required in "$LC_APP_SOURCE" "$LC_APP_LIST_SOURCE" "$LC_BOOTSTRAP_SOURCE"; do
  if [[ -z "$required" ]]; then
    echo "A required LiveContainer source file could not be located." >&2
    exit 1
  fi
done
...
text = path.read_text()
replacements = {
    'for(int i = 0; i < header->ncmds > 0; ++i) {': 'for(int i = 0; i < header->ncmds; ++i) {',
    'static BOOL checkJITEnabled() {': 'static BOOL checkJITEnabled(void) {',
}
...
private enum AndroidPreloadedGuestInstaller {
    ...
    static func installIfNeeded(fileManager: FileManager = .default) throws {
        ...
        appInfo["LCContainers"] = [["folderName": containerFolderName, "name": "Android"]]
        appInfo["isJITNeeded"] = false
        appInfo["dontInjectTweakLoader"] = true
        appInfo["dontLoadTweakLoader"] = true
        let appInfoData = try PropertyListSerialization.data(fromPropertyList: appInfo, format: .binary, options: 0)
        ...
        let containerInfoData = try PropertyListSerialization.data(fromPropertyList: containerInfo, format: .binary, options: 0)
        try containerInfoData.write(to: container.appendingPathComponent("LCContainerInfo.plist"), options: .atomic)
...
comment = '            // load apps\n'
inserted = '            try AndroidPreloadedGuestInstaller.installIfNeeded(fileManager: fm)\n' + comment
if inserted not in text:
    if comment not in text:
        raise SystemExit('Could not find LiveContainer app-loading block')
    text = text.replace(comment, inserted, 1)
path.write_text(text)
PY
...
test -f "$HOST_APP/$HOST_EXECUTABLE"
test -f "$HOST_APP/PreloadedApps/UTM SE.app/$UTM_EXECUTABLE"
...
(cd "$WORK/package" && zip -qry "$OUT/$OUTPUT_IPA" Payload)
